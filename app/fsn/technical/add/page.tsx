//app/fsn/technical/add/page.tsx
'use client';
...
const FSNPage = () => {
  ...
  const feasibilityOptions = [
    { value: '' as const, label: 'Select Feasibility', color: 'bg-gray-500' },
  ...
  // Fetch enquiries on component mount (only after mounted)
  useEffect(() => {
    if (!mounted) return;
    fetchEnquiries();
  ...
  const handleEnquiryChange = async (enquiryId: string) => {
    ...
    try {
      ...
      // Format the date to YYYY-MM-DD format for date input
      ...
      setFsnData(prev => ({
        ...
        fsn_enquiry_date: formatDateForInput(enquiryData.created_on),
  ...
  const handleSubmitFSN = async (sendToTechnical: boolean) => {
    try {
      ...
      console.log('Submitting FSN payload:', payload);
  ...
  return (
    <div className="container text-sm">
      ...
      <div className="space-y-2">
                  ...
                  {products.map((product) => (
                    <tr key={product.id}>
                      ...
                      <td className="px-2 py-1 whitespace-nowrap">
                        {product.fsn_feasibility ? (
                          <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full text-white ${getFeasibilityColor(product.fsn_feasibility)}`}>
                            {feasibilityOptions.find(opt => opt.value === product.fsn_feasibility)?.label || 'N/A'}
                          </span>
                        ) : (
                      ...
                      <td className="px-2 py-1 whitespace-nowrap text-gray-500">
                        {product.fsn_bom_cost > 0 ? `₹${product.fsn_bom_cost.toFixed(2)}` : '-'}
                      </td>
        ...
        <div className="flex justify-center gap-2 pt-6">
          ...
          <button
            ...
            onClick={() => {
              if (typeof window !== 'undefined') {
                window.location.href = '/fsn/technical/list/';
              }
            }}
      ...
      {isEditModalOpen && editingProduct && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-4 w-full max-w-md m-4 max-h-[85vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-3">
              <h2 className="text-lg font-semibold">Add FSN Details</h2>
              <button
            ...
            <div className="space-y-3 text-sm">
              ...
              <div>
                <label className="block font-medium mb-1">Feasibility *</label>
                <select
                  value={editingProduct.fsn_feasibility}
                  onChange={(e) =>
                    setEditingProduct((prev) =>
                      prev ? { ...prev, fsn_feasibility: e.target.value as any } : null
                    )
                  }
              ...
              <div>
                ...
                <input
                  ...
                  step="0.01"
                  min="0"
                  value={editingProduct.fsn_bom_cost}
                  onChange={(e) =>
                    setEditingProduct((prev) =>
                      prev ? { ...prev, fsn_bom_cost: parseFloat(e.target.value) || 0 } : null
                    )
                  }
              ...
              <div className="flex justify-center gap-3 pt-4">
                <button
                  ...
                  disabled={!editingProduct.fsn_feasibility || editingProduct.fsn_bom_cost <= 0}
                  className="bg-blue-600 text-white px-6 py-2 rounded-md hover:bg-blue-700 transition font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Save
                </button>
                <button
